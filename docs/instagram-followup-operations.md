...
3. Inspect `follow_up_due_at`, `follow_up_claimed_at`, `follow_up_sent_at`, `follow_up_cancelled_at`, `follow_up_attempts`, and `follow_up_last_error` in `instagram_conversations`.
4. Verify that the connected account token and `instagram_business_manage_messages` permission are still valid.
