...
## 2026-06-15 - [Security: Addressing False Positives in CI Security Checks]

**Learning:** ESLint rules like `security/detect-object-injection` and `security/detect-non-literal-fs-filename` can produce false positives when reading dynamic keys or files that are internally controlled (e.g., in test mocks or build scripts). These strict rules will fail the CI pipeline (`eslint . --max-warnings 0`) if left unaddressed.
**Action:** When safe dynamic inputs trigger security rule false positives, use localized `// eslint-disable-next-line security/detect-XYZ` comments rather than disabling the rule project-wide or ignoring CI failures.
