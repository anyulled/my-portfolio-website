...
const processGCSFiles = async (files: StorageFileLike[], limit?: number) => {
...
const logGCSError = (error: unknown, bucketName: string, prefix: string) => {
...
const fetchPhotosFromGCS = async (
  ...
  try {
    const files = await getGCSFiles(bucketName, prefix, limit, storageClient);
    ...
    return await processGCSFiles(files, limit);
  } catch (error) {
    logGCSError(error, bucketName, prefix);
    return null;
  }
};
