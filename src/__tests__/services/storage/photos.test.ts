...
describe("Photos Storage Service", () => {
    ...
    const mockFile = {
        ...
        getSignedUrl: jest.fn().mockResolvedValue(["https://signed-url.com/photo.jpg"]),
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        (Storage as unknown as jest.Mock).mockImplementation(() => mockStorageClient);
    });
    ...
    it("should return cached data from Vercel Blob if Redis misses", async () => {
        ...
        expect(setRedisCachedData).toHaveBeenCalledWith(expect.anything(), [mockPhoto], expect.any(Number));
    });
    ...
    it("should exclude photos with invalid ID", async () => {
        ...
        const invalidFile = { ...mockFile, metadata: { metadata: { id: "invalid" } } };
