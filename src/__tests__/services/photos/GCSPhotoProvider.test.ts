...
import {
  GCSPhotoProvider,
  createGCSPhotoProvider,
} from "@/services/photos/GCSPhotoProvider";
...
const createMockFile = (
  ...
  getSignedUrl: jest
    .fn()
    .mockResolvedValue([`https://signed.example.com/test-bucket/${name}`]),
  ...overrides,
});
...
describe("GCSPhotoProvider", () => {
  ...
  describe("listPhotos", () => {
    ...
    it("filters by prefix", async () => {
      ...
      expect(mockBucketContainer.getFiles).toHaveBeenCalledWith({
        ...
        autoPaginate: false,
        // Default limit is 100, buffer is 20
        maxResults: 120,
      });
    });
    ...
    it("sorts by date descending by default", async () => {
      const mockFiles = [
        createMockFile("old_1_o.jpg", {
          metadata: { updated: "2020-01-01T00:00:00Z" },
        }),
        createMockFile("new_2_o.jpg", {
          metadata: { updated: "2025-01-01T00:00:00Z" },
        }),
        createMockFile("mid_3_o.jpg", {
  ...
  describe("URL handling", () => {
    ...
    it("uses the file public URL even when signing is available", async () => {
